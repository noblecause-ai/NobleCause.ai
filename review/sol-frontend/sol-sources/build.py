#!/usr/bin/env python3
"""Deterministic static renderer for NobleCause.

Reads data/sitzung.json, validates it, and writes index.html.
The output contains all essential content for browsers without JavaScript.
"""
...
DATA_PATH = ROOT / "data" / "sitzung.json"
SCHEMA_PATH = ROOT / "schema" / "sitzung.schema.json"
...
def focus_recommendation(session: dict[str, Any]) -> dict[str, Any]:
    """Deterministic rule from the brief: vote share, confidence, pillar."""
...
def recommendation_card(session: dict[str, Any], rec: dict[str, Any], focused: bool = False) -> str:
    pillar = rec["pillar"]
    label = session["presentation"]["pillar_labels"][pillar]
    symbol = session["presentation"]["pillar_symbols"][pillar]
    ...
    if rec["has_consensus"]:
        ...
        if conditional_count:
            ...
            if pillar == "A":
                revision_html = (
                    '<div class="revision-note" aria-label="Änderung aus der vorigen Beratung">'
                    '<span class="revision-label">Was sich geändert hat</span>'
                    '<span class="chalk-strike">Pratham / TaRL Africa</span>'
                    '<span aria-hidden="true"> → </span><strong>Helen Keller International</strong>'
                    '<small>Sitzung 3, nach Gegenlesen revidiert</small>'
                    '</div>'
                )
        return f'''
        <article class="{' '.join(classes)}" id="empfehlung-{pillar.lower()}">
          ...
          <p class="recommendation-title">{esc(rec["title"])}</p>
          <p class="plain-reason">{esc(rec["rationale_md"])}</p>
...
def round_by_kind(session: dict[str, Any], kind: str) -> dict[str, Any]:
    return next(r for r in session["rounds"] if r["kind"] == kind)
...
def protocol_column(session: dict[str, Any], vote: dict[str, Any], phase: str, initial_vote: dict[str, Any] | None = None) -> str:
    ...
    for pillar in ["A", "B", "C", "D"]:
        ...
        conditional = rec.get("conditional")
        ...
        rows.append(f'''
          <section class="vote-row" id="model-{esc(family)}-{esc(phase)}-{pillar.lower()}">
            ...
            <p>{esc(rec["title"])}</p>
            {f'<p class="condition-inline">Unter Vorbehalt: {esc(reservation)}</p>' if conditional and reservation else ''}
          </section>
        ''')
    return f'''
...
def render() -> str:
    session = json.loads(DATA_PATH.read_text(encoding="utf-8"))
    ...
    initial = round_by_kind(session, "initial_vote")
    final = round_by_kind(session, "final_vote")
    ...
    second_columns = "".join(protocol_column(session, v, "final", initial_by_model[v["model"]]) for v in final["votes"])
    ...
    archive = "".join(
        ...
        '''
        for item in session["presentation"]["archive"]
    )
  ...
  <main>
    <section class="hero" aria-labelledby="hero-title">
      ...
      <div class="hero-content">
        <p class="context-line">Drei künstliche Intelligenzen beraten öffentlich, wohin Spenden am wirksamsten fließen. Jede Sitzung wird vollständig veröffentlicht.</p>
        ...
        <div class="lit-floor" id="antworten">
          ...
          <h1 id="hero-title">{esc(focus['organization'])}</h1>
          <p class="focus-reason">{esc(focus['rationale_md'])}</p>
