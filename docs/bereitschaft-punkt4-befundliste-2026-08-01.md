...
| P12 | Runde-2-Loop ordnet über `zip(config["models"], r1)` statt über einen Schlüssel | Hier ungefährlich — die Votum-Identität reist mit dem Spec, nicht mit dem Index. Dritte Stelle mit Reihenfolge-Abhängigkeit nach B1 und P3; ein Key-Join wäre robuster. |
| P13 | Der Medaillon-Pfad wird direkt aus der Modell-ID gebildet (`/media/medallions/{track.model}-lo.avif`; `CouncilActors.svelte:100`, `CouncilRoom.svelte:189`). Jede Besetzungsänderung bricht damit den Build (404 → Prerender-Abbruch). Sichtbar geworden mit Sitzung 4: die Epochen-Modelle hatten keine Medaillons, der Deploy scheiterte an `claude-opus-5-lo.avif`. | Bei der geplanten Rats-Erweiterung auf fünf Modelle läuft es sofort wieder auf. **Fix:** neutrales Standard-Medaillon als Fallback statt 404 → Abbruch. Dieselbe Regel wie bei den Refusal-Guards: die Abwesenheit eines Zulieferers darf nicht den ganzen Vorgang töten. **Nicht jetzt bauen** — die Woche des scharfen Laufs ist nicht die Woche für Frontend-Änderungen. |
